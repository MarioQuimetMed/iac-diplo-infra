...
export class InventoriesService {
  ...
  async handleReservationRequested(payload: ReservationRequestedEvent) {
    this.logger.log(
      `Processing reservation requested event for reservation ${payload.reservationId}`,
    );
    ...
    if (!room) {
      this.logger.warn(
        `Room ${roomId} not found for reservation ${reservationId}`,
      );
    ...
    if (overlappingBlock) {
      this.logger.warn(
        `Room ${roomId} is unavailable for dates ${checkInDate} to ${checkOutDate}. Reservation ${reservationId} rejected.`,
      );
      this.natsClient.emit(ROOM_UNAVAILABLE_EVENT, { reservationId });
    } else {
      this.logger.log(
        `Room ${roomId} is available. Blocking dates for reservation ${reservationId}.`,
      );
