...
export class ReservationsService {
  ...
  async create(
    ...
    this.logger.log(
      `Creating reservation for room ${createReservationDto.roomId}...`,
    );
    ...
    this.logger.log(
      `Reservation ${savedReservation.id} saved as PENDING. Emitting event to NATS...`,
    );

    this.natsClient.emit<any, ReservationRequestedEvent>(
      RESERVATION_REQUESTED_EVENT,
      {
  ...
  async updateStatus(
    ...
    if (status === ReservationStatus.CONFIRMED) {
      ...
      this.logger.log(
        `Emitted ${NOTIFY_GUEST_EVENT} to NATS for reservation ${id}`,
      );
    }
